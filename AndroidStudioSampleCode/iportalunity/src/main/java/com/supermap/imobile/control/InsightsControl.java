...
import com.supermap.imobile.adapter.InsightsAdapter;
...
public class InsightsControl {
    ...
    public InsightsControl(Context context, ListView listView, PullToRefreshView pullToRefreshView) {
    ...
    public void getInsights() {
        try {
            ...
            IPortalService.getInstance().getInsights(searchParameter,listener);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private OnResponseListener listener = new OnResponseListener() {
        ...
        public void onComplete(final Response response) {
            ...
            if (adapter == null) {
                adapter = new InsightsAdapter(mMainActivity, insightsBean);
            } else {
