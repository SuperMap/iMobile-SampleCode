...
import com.supermap.imobile.adapter.MyMapDashboardsAdapter;
 ...
 * 我的洞察
 */
public class MyMapDashboardsControl {
    private static final String TAG = "MyMapDashboardsControl";

    private MainActivity mMainActivity = null;
    private ListView mListView = null;
    private PullToRefreshView mPullToRefreshView;
    private MyMapDashboardsAdapter adapter = null;

    public MyMapDashboardsControl(Context context, ListView listView, PullToRefreshView pullToRefreshView) {
     ...
     * 联网请求更新数据
     */
    public void getMyMapDashboards() {
        try {

            HashMap<String, String> searchParameter = new HashMap<>();
            searchParameter.put("pageSize", "20");
            IPortalService.getInstance().getMyMapDashboards(searchParameter,listener);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private OnResponseListener listener = new OnResponseListener() {
        ...
        public void onComplete(final Response response) {
            ...
            if (adapter == null) {
                adapter = new MyMapDashboardsAdapter(mMainActivity, myMapDashboards);
            } else {
