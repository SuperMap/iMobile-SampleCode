    ...
    private void locateMyPosition() {
        ...
        if (contains && point2D.getX() != 0 && point2D.getY() != 0) {
            if (this.listener != null) {
                listener.locationChangedListener(point2D, mAccuracy, mBearing, (float) m_GPSData.dSpeed);
            }
        }
    }
...
//        Log.d(TAG , "name: " + name + ", status: " + status + ", desc: " + desc);
    ...
    public interface LocationChangedListener {
        void locationChangedListener(Point2D point2D, double accuracy, float bearing, float speed);
    }
