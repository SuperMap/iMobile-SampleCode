...
public class CompassView extends View{
    ...
    public CompassView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        mLongLatPaint=new Paint();
        mLongLatPaint.setStyle(Paint.Style.STROKE);
        mLongLatPaint.setAntiAlias(true);
        mLongLatPaint.setTextSize(50);
        mLongLatPaint.setColor(context.getResources().getColor(R.color.white));
...
//        drawCenterText();
    ...
    private void drawText() {
        if (val<=15||val>=345){
            text = "北"+String.valueOf((int)val)+"°";
        }else if (val>15&&val<=75){
            text= "东北"+String.valueOf((int)val)+"°";
        }else if (val>75&&val<=105){
            text= "东"+String.valueOf((int)val)+"°";
        }else if (val>105&&val<=165){
            text="东南"+String.valueOf((int)val)+"°";
        }else if (val>165&&val<=195){
            text = "南"+String.valueOf((int)val)+"°";
        }else if (val>195&&val<=255){
            text = "西南"+String.valueOf((int)val)+"°";
        }else if (val>255&&val<=285){
            text = "西"+String.valueOf((int)val)+"°";
        }else if (val>285&&val<345){
            text="西北"+String.valueOf((int)val)+"°";
        }
        ...
        //让文字水平居中显示
        mCanvas.drawText(text,width/2-mTextWidth/2,mTextHeight/3,mTextPaint);

    }

    private void drawLongLat() {

        text="纬度"+DDtoDMS(latitude)+"  经度"+DDtoDMS(longtitude);

        mLongLatPaint.getTextBounds(text,0,text.length(),mTextRect);
        //文字宽度
        int mTextWidth = mTextRect.width();
        //让文字水平居中显示
        mCanvas.drawText(text,width/2-mTextWidth/2,mTextHeight*2/3,mLongLatPaint);
